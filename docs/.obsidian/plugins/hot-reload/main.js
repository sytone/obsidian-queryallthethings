...
module.exports = class HotReload extends Plugin {
    ...
    async getPluginNames() {
        ...
        for (const { id, dir } of Object.values(app.plugins.manifests)) {
            ...
            if ((await this.app.vault.exists(dir + "/.git")) || (await this.app.vault.exists(dir + "/.hotreload"))) enabled.add(id);
        }
    ...
    onFileChange(filename) {
        ...
        const reloader = this.pluginReloaders[plugin] || (this.pluginReloaders[plugin] = this.debouncedMethod(750, this.requestReload, plugin));
        reloader();
    }
